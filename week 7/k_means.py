...
class KMeans(object):
  ...
  def train(self, X,y_train, epsilon=1e-12):
    """
    ...
    for i in range(len(y_train)):
        if (len(self.centroids[y_train[i]]) == 0):
            self.centroids[y_train[i]] = X[i]
    lists = [[],[],[]]
    while (all(num > epsilon for num in change)):
        ...
        #########################################################################
        d = self.compute_distances_no_loops(X)
        ...
        for i in range(self.num_clusters):
            new_centroids[i] = (sum(lists[i])/len(lists[i]))
        
        change = np.sqrt(np.sum((np.array(self.centroids)-np.array(new_centroids))**2,1))
